...
class Outlier_Removal(nn.Module):
    ...
    def forward(self, kpt_list, desc_list, batch):
        # kpts = {ndarray:(2000,2)}
        nkpts = [torch.from_numpy(normalize_kpts(i).astype(np.float32)).cuda() for i in kpt_list]
        ...
        y0 = y_hat[0][0, :].detach().cpu().numpy()
        y1 = y_hat[1][0, :].detach().cpu().numpy()
        inlier_idx = np.where(y1 > self.default_config.inlier_threshold)
        y = [y0[inlier_idx], y1[inlier_idx]]
        logits = torch.Tensor(np.array(y))
        ...
        desc0_orn = desc_list[0][matches[:, 0]]
        desc1_orn = desc_list[1][matches[:, 1]]
