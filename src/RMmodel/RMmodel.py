...
class RMmodel(nn.Module):
    ...
    def forward(self, data, online_resize=False):
        ...
        if data['hw0_i'] == data['hw1_i']:  # faster & better BN convergence
            ...
            # return 1/8的 coarse feature map feats_c, 1/2 fine feature map feats_f
        ...
        if (data['mkpts0_f'].tolist()).__len__() == 0:
            ...
            desc0 = [feat_r0[int(kpts0[i][0])][int(kpts0[i][1])] for i in range(len(kpts0))]
            desc1 = [feat_r1[int(kpts1[j][0])][int(kpts1[j][1])] for j in range(len(kpts0))]
