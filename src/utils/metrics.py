...
def ploting_compute_symmetrical_epipolar_errors(data):
    """
    Update:
        data (dict):{"epi_errs": [M]}
    """
    ...
    if (data['mkpts0_orn'].tolist()).__len__() == 0:
        pts0 = (torch.tensor(([], []))).t().cuda()
        pts1 = (torch.tensor(([], []))).t().cuda()
        # pts0 = data['mkpts0_f']
        # pts1 = data['mkpts1_f']
    else:
        pts0 = torch.from_numpy(data['mkpts0_orn']).cuda()
        pts1 = torch.from_numpy(data['mkpts1_orn']).cuda()
