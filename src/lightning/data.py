...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        self.coarse_scale = 1 / config.RMmodel.RESOLUTION[0]  # 0.125. for training RMmodel.
    ...
    def _setup_dataset(self,
        ...
        if mode == 'train':
            ...
            # local_npz_names = local_npz_names[:1]  # 仅用于测试，只加载前1个数据
        else:
            local_npz_names = npz_names
        logger.info(f'[rank {self.rank}]: {len(local_npz_names)} scene(s) assigned.')
        
        dataset_builder = self._build_concat_dataset_parallel if self.parallel_load_data else self._build_concat_dataset
        return dataset_builder(data_root, local_npz_names, split_npz_root, intri_path, mode=mode,
                               min_overlap_score=min_overlap_score, pose_dir=pose_dir)
    ...
    def train_dataloader(self):
        ...
        logger.info(f'[rank:{self.rank}/{self.world_size}]: '
                    f'Train Sampler and DataLoader re-init (should not re-init between epochs!).')
    ...
    def val_dataloader(self):
        ...
        if not isinstance(self.val_dataset, abc.Sequence):
            ...
            sampler = DistributedSampler(self.val_dataset)  # 限制数据加载到数据集子集的采样器
